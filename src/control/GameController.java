...
/**
 * Created by Tong on 12.04. GameController, dealing with logic along game
 * process.
 */
...
//		Node node = minimax.search(board);
//
////        view.movePieceFromAI(result.piece, result.to);
////        board.updatePiece(result.piece, result.to);
//		view.movePieceFromAI(node.piece, node.to);
//		board.updatePiece(node.piece, node.to);
//
//	}
	public void responseMoveChessMinimaxOnePlayer(Board board, GameView view, int depth) {
		Minimax minimax = new Minimax();
		Node node = minimax.search(board,depth);

		view.movePieceFromAI(node.piece, node.to);
		board.updatePiece(node.piece, node.to);
	}

	public void responseMoveChessABOnePlayer(Board board, GameView view, int depth) {
		SearchModel searchModel = new SearchModel();
		AlphaBetaNode result = searchModel.search(board, depth);
	...
	public void printBoard(Board board) {
		/**
		 * Piece position is stored internally as [row, col], but output standard
		 * requires [col,row]. Here comes the conversion. eg. [0, 4] --> [E, 0]
		 */
	...
	public boolean hasWinGame(Board board) {
